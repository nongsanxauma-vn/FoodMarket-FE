/**
 * ChatBot Modal Component
 * Modal overlay with chat interface
 */

import React, { useEffect, useRef } from 'react';
import { useChatBotContext } from './ChatBotContext';
import OptimizedMessageInput from './OptimizedMessageInput';
import VirtualizedMessageList from './VirtualizedMessageList';
import { TypingIndicator } from './TypingIndicator';
import { SuggestionsPanel } from './SuggestionsPanel';
import { 
  useFocusManagement, 
  useScreenReader, 
  useHighContrast, 
  useReducedMotion, 
  useAriaLiveRegion 
} from './hooks/useAccessibility';
import './ChatBot.css';

interface ChatBotModalProps {
  title?: string;
  className?: string;
}

export function ChatBotModal({ 
  title = 'AI Assistant', 
  className = '' 
}: ChatBotModalProps) {
  const { 
    isOpen, 
    toggleChat, 
    error, 
    messages, 
    isLoading, 
    sendMessage,
    suggestions,
    selectSuggestion
  } = useChatBotContext();
  
  // Accessibility hooks
  const { containerRef, focusFirst } = useFocusManagement(isOpen, true, toggleChat);
  const { announce } = useScreenReader();
  const { isHighContrast } = useHighContrast();
  const { prefersReducedMotion } = useReducedMotion();
  const { message: liveMessage, updateMessage } = useAriaLiveRegion();

  // Announce new messages to screen readers
  useEffect(() => {
    if (messages.length > 0) {
      const lastMessage = messages[messages.length - 1];
      if (lastMessage.sender === 'ai') {
        announce(`AI response: ${lastMessage.content}`, 'polite');
        updateMessage(`New message from AI assistant`, 'polite');
      }
    }
  }, [messages, announce, updateMessage]);

  // Announce loading state
  useEffect(() => {
    if (isLoading) {
      announce('AI is thinking...', 'polite');
      updateMessage('AI is processing your request', 'polite');
    }
  }, [isLoading, announce, updateMessage]);

  // Handle escape key to close modal (now handled by useFocusManagement, but keeping for custom scroll management)
  useEffect(() => {
    if (isOpen) {
      // Prevent body scroll when modal is open
      document.body.classList.add('chatbot-no-scroll');
    }

    return () => {
      document.body.classList.remove('chatbot-no-scroll');
    };
  }, [isOpen]);

  // Handle backdrop click to close modal
  const handleBackdropClick = (event: React.MouseEvent<HTMLDivElement>) => {
    if (event.target === event.currentTarget) {
      toggleChat();
    }
  };

  if (!isOpen) {
    return null;
  }

  return (
    <div
      className={`
        chatbot-modal-overlay
        ${isOpen ? 'is-open' : ''}
        ${className}
      `}
      onClick={handleBackdropClick}
      style={{
        opacity: prefersReducedMotion ? 1 : undefined,
        transition: prefersReducedMotion ? 'none' : undefined
      }}
    >
      {/* Modal Container */}
      <div
        ref={containerRef as React.RefObject<HTMLDivElement>}
        className={`
          chatbot-modal
          ${isOpen ? 'is-open' : ''}
          ${isHighContrast ? 'is-high-contrast' : ''}
        `}
        role="dialog"
        aria-modal="true"
        aria-labelledby="chatbot-title"
        aria-describedby="chatbot-description"
        tabIndex={-1}
        onClick={(e) => e.stopPropagation()}
      >
        {/* Decorative Background Elements */}

        {/* Header */}
        <div className="chatbot-modal-header">
          <div className="chatbot-modal-title">
            {/* AI Avatar */}
            <div className="chatbot-avatar" aria-hidden="true">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <path d="M12 2a10 10 0 1 0 10 10A10 10 0 0 0 12 2zm0 18a8 8 0 1 1 8-8 8 8 0 0 1-8 8z"></path>
                <path d="M12 6a6 6 0 1 0 6 6 6 6 0 0 0-6-6zm0 10a4 4 0 1 1 4-4 4 4 0 0 1-4 4z"></path>
                <circle cx="12" cy="12" r="2"></circle>
              </svg>
            </div>
            
            {/* Title and Status */}
            <div className="chatbot-title-text">
              <h2 id="chatbot-title">
                {title}
              </h2>
              <p id="chatbot-status">
                Online and ready to help
              </p>
            </div>
          </div>

          {/* Close Button */}
          <button
            onClick={toggleChat}
            className="chatbot-close-button"
            aria-label="Close chat assistant"
            title="Close chat"
          >
            <svg
              width="20"
              height="20"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
              aria-hidden="true"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth={2}
                d="M6 18L18 6M6 6l12 12"
              />
            </svg>
          </button>
        </div>

        {/* Screen Reader Only Description */}
        <div id="chatbot-description" className="sr-only" style={{ position: 'absolute', left: '-10000px', width: '1px', height: '1px', overflow: 'hidden' }}>
          AI-powered chat assistant. Use Tab to navigate between controls.
        </div>

        {/* Live Region for Screen Reader Announcements */}
        <div aria-live="polite" aria-atomic="true" className="sr-only" style={{ position: 'absolute', left: '-10000px', width: '1px', height: '1px', overflow: 'hidden' }}>
          {liveMessage}
        </div>

        {/* Error Banner */}
        {error && (
          <div className="chatbot-error-banner" role="alert">
            <div className="flex">
              <div className="flex-shrink-0">
                <svg
                  width="20"
                  height="20"
                  fill="currentColor"
                  viewBox="0 0 20 20"
                  aria-hidden="true"
                >
                  <path
                    fillRule="evenodd"
                    d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z"
                    clipRule="evenodd"
                  />
                </svg>
              </div>
              <div style={{ marginLeft: '12px' }}>
                <p>{error.message}</p>
              </div>
            </div>
          </div>
        )}

        {/* Chat Content Area */}
        <div className="chatbot-modal-content" role="main">
          {/* Messages Area */}
          <div className="chatbot-messages-area" role="log" aria-label="Chat conversation">
            {messages.length === 0 ? (
              <div className="chatbot-welcome text-center py-8">
                <div className="chatbot-welcome-icon mx-auto mb-4">
                  <svg
                    width="48"
                    height="48"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                    aria-hidden="true"
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      strokeWidth={2}
                      d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"
                    />
                  </svg>
                </div>
                <p style={{ fontSize: '0.875rem', marginBottom: '8px' }}>
                  Welcome! I'm here to help you with products, orders, and more.
                </p>
                <p style={{ fontSize: '0.75rem', opacity: 0.7 }}>
                  Start typing to begin our conversation...
                </p>
              </div>
            ) : (
              <VirtualizedMessageList 
                messages={messages}
                isLoading={isLoading}
                onLoadMore={() => {}}
                style={{ flex: 1 }}
              />
            )}
            
            {/* Typing Indicator */}
            {isLoading && <TypingIndicator isVisible={true} />}
          </div>

          {/* Suggestions Panel */}
          {suggestions.length > 0 && (
            <SuggestionsPanel
              suggestions={suggestions}
              onSelectSuggestion={selectSuggestion}
              isVisible={!isLoading}
            />
          )}

          {/* Input Area */}
          <OptimizedMessageInput
            onSendMessage={sendMessage}
            disabled={isLoading}
            placeholder="Type your message..."
          />
        </div>
      </div>
    </div>
  );
}