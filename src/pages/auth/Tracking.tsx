...
const Tracking: React.FC<TrackingProps> = ({ onBack, orderId: orderIdProp, orderCode }) => {
  ...
  // Shipper đang lấy hàng: xanh biển nhấp nháy khi SHIPPING
  ...
  const updateMarker = useCallback((lat: number, lng: number) => {
    if (shipperMarkerRef.current) shipperMarkerRef.current.setLatLng([lat, lng]);
  ...
  const drawRoutes = useCallback(async (
    ...
    // ✅ Đã có route → chỉ trim (FakeGPS) hoặc bỏ qua (GPS thật)
    if (fullRoute1Ref.current.length > 0 || fullRoute2Ref.current.length > 0) {
      if (isFake) trimRoute(sLat, sLng, shopLat, shopLng);
      return;
    }

    // ✅ Chưa có route → fetch từ OSRM (chỉ 1 lần)
    // Trong khi đang fetch, các update tiếp theo sẽ thấy fullRoute vẫn rỗng
    // → dùng flag để tránh fetch nhiều lần song song
  ...
  const clearMapLayers = useCallback(() => {
    ...
    // ✅ Reset fetch flag để vẽ lại route khi shipper bật GPS trở lại
    (drawRoutes as any)._fetching = false;
  }, []);
    ...
    (async () => {
      try {
        ...
        if (res.result) {
          ...
          setLocation({ lat: loc.latitude, lng: loc.longitude, updatedAt: loc.updatedAt, shipperName: loc.shipperName, shipperId: loc.shipperId, shopLat: loc.shopLatitude, shopLng: loc.shopLongitude, destLat: loc.destLatitude, destLng: loc.destLongitude });
          setShipperActive(true); shipperActiveRef.current = true;
          ...
          if (loc.shopLatitude && loc.shopLongitude && loc.destLatitude && loc.destLongitude) {
            setTimeout(() => drawRoutes(loc.latitude, loc.longitude, loc.shopLatitude!, loc.shopLongitude!, loc.destLatitude!, loc.destLongitude!, false), 600);
          }
        }
      } catch {
  ...
  useEffect(() => {
    return () => {
      ...
      shipperMarkerRef.current = null; routeLayer1Ref.current = null; routeLayer2Ref.current = null;
    };
  }, []);
  ...
  return (
    <div className="flex-1 bg-background animate-in fade-in duration-500 pb-20">
      {/*
        ✅ FIX MAP TRÀN HEADER:
        Toàn bộ trang wrap trong position:relative + overflow:hidden
        Map container dùng isolation để tạo stacking context riêng
      */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-4 flex flex-col gap-8">
                ...
                {steps.map((step, idx) => (
                  <div key={idx} className="relative flex items-start gap-6">
                    <div className={`size-12 rounded-2xl flex items-center justify-center z-10 shadow-sm transition-all ${
                      ...
                      step.done    ? 'bg-primary text-white' :          // xanh lá
                      step.active  ? 'bg-blue-500 text-white animate-pulse' :  // xanh biển nhấp nháy
            ...
            <div className="bg-white rounded-[40px] border border-gray-100 shadow-sm p-8 flex flex-col gap-6">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    {location && <p className="text-[10px] text-green-500 font-bold mt-1 flex items-center gap-1"><Navigation className="size-3" /> Cập nhật lúc {fmt(location.updatedAt)}</p>}
                  </div>
                </div>
                <div className="flex gap-2">
                  ...
                  {/* ✅ Chat với shipper — giống ProductDetail chat với shop */}
                  <button
                    onClick={() => {
              ...
              {location && <div className="bg-gray-50 rounded-2xl p-4 border border-gray-100"><p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-2">Vị trí hiện tại</p><p className="text-xs font-mono text-gray-600">{location.lat.toFixed(6)}, {location.lng.toFixed(6)}</p></div>}
              {error && <div className="flex items-center gap-2 bg-yellow-50 text-yellow-700 rounded-2xl p-4 border border-yellow-100"><AlertCircle className="size-4 shrink-0" /><p className="text-xs font-bold">{error}</p></div>}
              {location && !shipperActive && lastUpdateRef.current && (
                <div className="flex items-center gap-2 bg-gray-50 text-gray-500 rounded-2xl p-4 border border-gray-200"><WifiOff className="size-4 shrink-0" /><p className="text-xs font-bold">Shipper đã tắt GPS — vị trí hiển thị lần cuối</p></div>
              )}
            </div>
            )}
          </div>
          ...
          <div className="lg:col-span-8 flex flex-col gap-8">
            ...
            {/*
              ✅ FIX MAP TRÀN HEADER:
              - Wrapper: position relative, KHÔNG có overflow
              - Inner map div: overflow hidden + clip-path để cắt map
              - Buttons: nằm trên wrapper (outside overflow), z-index cao
            */}
            <div className="relative rounded-[40px] shadow-lg border border-gray-100" style={{ height: '480px' }}>

              {/* Map container — overflow hidden clip map, không tràn ra ngoài */}
              <div
              ...
              {/* Badge Live — nằm ngoài overflow, không bị clip */}
              {wsStatus === 'connected' && (
              ...
              {/* Nút Follow — nằm ngoài overflow, luôn hiển thị */}
              <button
                onClick={() => setFollowShipper(prev => {
                  const next = !prev;
                  if (next && location && leafletMapRef.current) leafletMapRef.current.panTo([location.lat, location.lng], { animate: true });
                  return next;
                })}
            ...
            {/* ✅ TÓM TẮT ĐƠN HÀNG từ API — giống MyOrders */}
            <div className="bg-white rounded-[40px] border border-gray-100 shadow-sm p-10">
              ...
              {loadingOrder ? (
                ...
                <>
                  {/* Items — dùng layout giống MyOrders */}
