...
const OrderPreparation: React.FC<OrderPreparationProps> = ({ orderId, onBack, onComplete }) => {
  ...
  const nearbyShippers = [
    { id: 'shipper-1', name: 'Anh Minh - Shipper', rating: 4.8, reviews: 142, distance: '0.8 km', phone: '0868 123 456', vehicle: 'Honda Wave Blue', status: 'Có sẵn', avgDeliveryTime: '20 phút' },
    { id: 'shipper-2', name: 'Chị Lan - Shipper', rating: 4.9, reviews: 215, distance: '1.2 km', phone: '0912 789 012', vehicle: 'Xe máy SH', status: 'Có sẵn', avgDeliveryTime: '25 phút' },
    { id: 'shipper-3', name: 'Anh Hoa - Shipper', rating: 4.7, reviews: 98, distance: '1.5 km', phone: '0901 234 567', vehicle: 'Xe tải nhỏ', status: 'Bận', avgDeliveryTime: '35 phút' },
  ];
      ...
      <div className="w-full max-w-6xl mx-auto px-4 md:px-6 py-8">
        ...
        <div className="bg-white rounded-[32px] border border-gray-100 p-8 mb-8">
          ...
          <div className="space-y-4">
            {[
              { method: 'self', label: 'Tôi có phương tiện giao hàng', sub: 'Tự vận chuyển đến khách hoặc liên hệ shipper', icon: Truck },
              { method: 'shipper', label: 'Tôi không có phương tiện - Tìm shipper gần', sub: 'Hệ thống sẽ gợi ý shipper có sẵn trong bán kính gần', icon: MessageCircle },
            ].map(({ method, label, sub, icon: Icon }) => (
              <label
                key={method}
                className="flex items-start gap-4 p-4 border-2 border-gray-200 rounded-xl hover:border-primary hover:bg-green-50 cursor-pointer transition-all group"
                onClick={() => { setShipmentMethod(method as any); setSelectedShipperId(null); }}
              >
                <input type="radio" checked={shipmentMethod === method} onChange={() => {}} className="mt-1" />
                <div className="flex-1">
                  <p className="font-bold text-gray-900 group-hover:text-primary transition-colors">{label}</p>
                  <p className="text-xs text-gray-500 mt-1">{sub}</p>
                </div>
                <Icon className="size-5 text-gray-400 group-hover:text-primary transition-colors mt-1" />
              </label>
            ))}
          </div>
        </div>
          ...
          <div className="bg-white rounded-[32px] border border-gray-100 p-8 mb-8 animate-in fade-in">
            ...
            <div className="space-y-4 mb-6">
              <div>
                ...
                <select className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm font-bold">
              ...
              <div>
                ...
                <input type="time" className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm font-bold" defaultValue="14:30" />
              </div>
              <div>
                <label className="text-xs font-black text-gray-700 uppercase mb-2 block">Ghi chú cho khách hàng</label>
                <textarea className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm font-bold" placeholder="VD: Giao tại cửa trước..." rows={3} />
              </div>
            </div>
            <button onClick={handleConfirmShipment} disabled={updating} className="w-full bg-primary hover:bg-primary/90 text-white px-6 py-4 rounded-2xl font-black transition-all flex items-center justify-center gap-3 disabled:opacity-50">
          ...
          <div className="bg-white rounded-[32px] border border-gray-100 p-8 mb-8 animate-in fade-in">
            ...
            <div className="space-y-4">
              {nearbyShippers.map((shipper) => (
                <div key={shipper.id} onClick={() => setSelectedShipperId(shipper.id)}
                  className={`p-6 rounded-2xl border-2 cursor-pointer transition-all ${selectedShipperId === shipper.id ? 'border-primary bg-green-50' : 'border-gray-200 hover:border-primary/50 hover:bg-gray-50'}`}
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <h4 className="text-sm font-black text-gray-900">{shipper.name}</h4>
                        <span className={`px-2 py-1 text-[9px] font-black rounded-lg ${shipper.status === 'Có sẵn' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>{shipper.status}</span>
                      </div>
                      <div className="flex items-center gap-4 text-xs font-bold text-gray-600 mb-3">
                        <div className="flex items-center gap-1"><Star className="size-3.5 fill-yellow-400 text-yellow-400" /><span>{shipper.rating} ({shipper.reviews})</span></div>
                        <div className="flex items-center gap-1"><MapPin className="size-3.5" /><span>{shipper.distance}</span></div>
                      </div>
                      <p className="text-[10px] font-bold text-gray-600"><span className="text-gray-400">Phương tiện:</span> {shipper.vehicle}</p>
                      <p className="text-[10px] font-bold text-gray-600 mt-1"><span className="text-gray-400">Thời gian giao trung bình:</span> {shipper.avgDeliveryTime}</p>
                    </div>
                  </div>
                  {selectedShipperId === shipper.id && (
                    <div className="pt-4 border-t border-primary/20 space-y-3 mt-4">
                      <a href={`tel:${shipper.phone}`} className="flex items-center justify-center gap-2 w-full bg-primary text-white px-4 py-3 rounded-xl font-black text-sm hover:bg-primary/90 transition-colors">
                        <Phone className="size-4" /> Gọi ngay: {shipper.phone}
                      </a>
                      <button onClick={handleConfirmShipment} disabled={updating} className="flex items-center justify-center gap-2 w-full bg-gray-100 text-gray-900 px-4 py-3 rounded-xl font-black text-sm hover:bg-gray-200 transition-colors disabled:opacity-50">
                        <CheckCircle2 className="size-4" /> {updating ? 'Đang xử lý...' : 'Xác nhận liên hệ shipper này'}
                      </button>
                    </div>
                  )}
                </div>
