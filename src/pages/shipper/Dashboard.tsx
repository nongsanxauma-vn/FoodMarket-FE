               ...
               <div className="bg-white w-full max-w-2xl rounded-[40px] shadow-2xl relative z-10 flex flex-col max-h-[80vh] overflow-hidden animate-in zoom-in-95 duration-300">
                  <div className="p-8 border-b border-gray-100 flex items-center justify-between">
                     <div>
                        ...
                        <p className="text-xs text-gray-400 font-bold uppercase tracking-widest mt-1">Gửi từ ban quản trị Xấu Mã</p>
                     </div>
                     <button
                        onClick={() => setIsNotificationsOpen(false)}
                        className="size-12 bg-gray-50 text-gray-400 rounded-2xl flex items-center justify-center hover:bg-gray-100 transition-all"
                     >
                        <X className="size-6" />
                     </button>
                  </div>
                  ...
                  <div className="flex-1 overflow-y-auto p-8 space-y-4">
                     {loadingNotifications && notifications.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-20 gap-4">
                           ...
                           <p className="text-xs font-black text-gray-400 uppercase tracking-widest">Đang cập nhật thông báo...</p>
                        </div>
                     ) : notifications.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-20 gap-4 text-center">
                           <div className="size-20 bg-gray-50 rounded-[32px] flex items-center justify-center text-gray-200">
                              <Bell className="size-10" />
                           </div>
                           <p className="text-xs font-black text-gray-400 uppercase tracking-widest">Bạn hiện không có thông báo nào</p>
                        </div>
                     ) : (
                        notifications.map((n) => (
                           <div
                              key={n.id}
                              className={`p-6 rounded-[32px] border transition-all flex gap-5 group relative ${!n.isRead ? 'bg-primary/5 border-primary/10' : 'bg-white border-gray-100 shadow-sm'}`}
                           >
                              <div className={`size-12 rounded-2xl flex items-center justify-center shrink-0 ${!n.isRead ? 'bg-primary text-white' : 'bg-gray-100 text-gray-400'}`}>
                                 {n.title.toLowerCase().includes('đơn hàng') ? <Package className="size-6" /> : <Info className="size-6" />}
                              </div>
